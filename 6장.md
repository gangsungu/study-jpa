...
+ 외래키는 항상 다(N)쪽에서 관리해야하는데, 반대로 One쪽에서 관리하는 구조
    ...
    - 일대다 단방향 연관관계는 @JoinColumn을 반드시 사용해야 한다
        ...
        UPDATE orders SET member_id = ? WHERE id = ?;
        ```
        + insert 후 update가 추가로 발생 > 성능 저하, 트랜잭션 단위에서 복잡한 흐름이 발생
...
### 일대다 연관관계(양방향)
+ 일대다 양방향은 공식적으로 존재하지 않는다 (JPA 스펙상 @OneToMany 주인 + @ManyToOne 주인 조합은 불가)
    - 대신 일대다 단방향(@OneToMany + @JoinColumn)에 @ManyToOne을 읽기 전용으로 하나 더 추가해 양방향처럼 흉내낸다
    ```java
    // Team (일(1)쪽) : @OneToMany가 외래 키(TEAM_ID)를 관리하는 주인
    @OneToMany
    @JoinColumn(name = "TEAM_ID")
    private List<Member> members = new ArrayList<>();

    // Member (다(N)쪽) : 같은 TEAM_ID를 읽기 전용으로만 매핑
    @ManyToOne
    @JoinColumn(name = "TEAM_ID", insertable = false, updatable = false)
    private Team team;
    // 두 매핑이 같은 외래 키(TEAM_ID)를 가리키므로, Member쪽을 읽기 전용(insertable/updatable=false)으로 두어
    // 외래 키 관리 주체를 @OneToMany 하나로 고정해야 충돌이 없다
    ```
+ 일대다 단방향 연관관계와 같은 단점(INSERT 후 추가 UPDATE 등)을 그대로 공유한다
    - 따라서 실무에서는 이 구조 대신 **다대일 양방향**(@ManyToOne이 주인)으로 설계하는 것이 정석이다
...
> **모든 것의 뿌리: "외래 키(FK)를 누가 들고 있냐"**

- **FK는 항상 다(N)쪽 테이블에 있다.** 회원 여러 명이 각자 "나는 어느 팀"을 적어두는 게 자연스럽기 때문. (팀 하나가 회원 100명을 한 칸에 못 적음)
- **연관관계의 주인 = DB에서 FK를 실제로 들고 있는 쪽.** 그래서 N쪽이 주인이 되는 게 정석.
    - 주인: `@JoinColumn`을 붙여 FK를 관리
    - 주인 아님: `mappedBy`를 붙여 "난 읽기만, FK 주인은 저 필드야"라고 표시

| 관계 | 핵심 직관 |
| --- | --- |
| **다대일(N:1)** | FK가 있는 N쪽이 주인. 가장 자연스럽고 정석. |
| **일대다(1:N) 단방향** | 1쪽이 남의 테이블(N) FK를 관리하는 역주행 구조 → 저장 시 INSERT 후 UPDATE가 추가로 나감(비효율). **실무에선 다대일 양방향으로 바꾼다.** |
| **일대일(1:1)** | 양쪽 다 '1'이라 FK 위치를 고를 수 있음. 보통 많이 쓰는 **주 테이블에 FK + UNIQUE 제약**. "대상 테이블 FK + 단방향"은 주인과 FK 위치가 어긋나서 JPA가 지원 X → 양방향 필수. |
| **다대다(N:N)** | DB엔 다대다가 없어 **중간 테이블로 쪼갬(= 1:N + N:1)**. `@ManyToMany`는 편하지만 중간 테이블에 추가 컬럼(수강일·성적 등)을 못 넣음 → **실무에선 중간 테이블을 직접 엔티티로 승격.** |
| **복합 키** | 컬럼 2개 이상이 모여 PK 역할. `@IdClass`(필드 개별) / `@EmbeddedId`(객체로 묶음). 둘 다 `equals()`·`hashCode()` 필수 — JPA가 "같은 키인지" 비교해야 하므로. |
